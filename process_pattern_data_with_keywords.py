...
verbe_pol = dict(pd.read_csv("resources/lexiques/verbe_polarité.csv",sep=";").iloc[:,:].values)
df["verbe_polarity"] = df.verbe.apply(lambda x: verbe_pol[x] if x in verbe_pol else 0)
